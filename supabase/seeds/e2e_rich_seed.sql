...
BEGIN
  ...
  UPDATE teachers SET video_platforms = ARRAY['ZOOM', 'VOOV']::video_platform[] WHERE id = v_james;
  UPDATE teachers SET video_platforms = ARRAY['VOOV']::video_platform[] WHERE id = v_emily;
  ...
  UPDATE students SET video_platforms = ARRAY['VOOV']::video_platform[] WHERE id = v_l_cn;
  UPDATE students SET gender = 'female' WHERE id IN (v_l_fresh, v_l_active, v_l_sib1, v_l_pending);
  UPDATE students SET gender = 'male' WHERE id IN (v_l_hold, v_l_pay, v_l_renew, v_l_cn, v_l_sib2);
  ...
  FROM public.e2e_match_dates(v_today + 1, ARRAY['Mon', 'Wed', 'Fri'], 8, 1) d;
  ...
  FROM public.e2e_match_dates(v_today + 1, ARRAY['Mon', 'Wed', 'Fri'], 8, 1)
  WHERE n = 1;
...
BEGIN
  ...
  SELECT count(*) INTO v_bad
  FROM lessons l
  WHERE l.teacher_id IN (public.e2e_id(1), public.e2e_id(2))
    AND NOT EXISTS (
      SELECT 1
      FROM teachers_weekly_availability a
      WHERE a.teacher_id = l.teacher_id
        AND a.day = CASE EXTRACT(ISODOW FROM (l.scheduled_at AT TIME ZONE 'Asia/Seoul'))::int
          WHEN 1 THEN 'Mon' WHEN 2 THEN 'Tue' WHEN 3 THEN 'Wed'
          WHEN 4 THEN 'Thu' WHEN 5 THEN 'Fri' WHEN 6 THEN 'Sat' ELSE 'Sun'
        END
        AND a.start_time = (l.scheduled_at AT TIME ZONE 'Asia/Seoul')::time
    );
  IF v_bad > 0 THEN
    RAISE EXCEPTION 'e2e seed: % lessons fall outside teacher availability', v_bad;
  END IF;
