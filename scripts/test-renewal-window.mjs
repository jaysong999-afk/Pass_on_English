...
async function main() {
  ...
  await test("한지호 enrollments show pending renewal hold without clicking 재수강", async () => {
    ...
    const lastEndKst = new Intl.DateTimeFormat("en-GB", {
      timeZone: "Asia/Seoul",
      hour: "2-digit",
      minute: "2-digit",
      hour12: false,
    ...
    if (lastEndKst !== "10:20") {
      throw new Error(`last lesson end should be 10:20 KST, got ${lastEndKst} (hold=${hold.paymentDeadlineAt})`);
    }
    return { note: `hold=${hold.id} deadline=${hold.paymentDeadlineAt} lastEndKst=${lastEndKst}` };
  });
  ...
  await test("student can renew before the last lesson and admin extends the schedule", async () => {
    ...
    try {
      ...
      const { data: original } = await db
        .from("enrollments")
        .select("status, sessions_total, sessions_remaining")
        .eq("id", enrollmentId)
        .maybeSingle();
      const { data: holdRow } = await db
        .from("enrollments")
        .select("status, cancel_reason, sessions_total, sessions_remaining, renewed_from_enrollment_id")
        .eq("id", holdId)
        .maybeSingle();
      ...
      const { count: renewalFutureCount } = await db
        .from("lessons")
